...
module.exports.run = async (bot, msg, args) => {
  ...
  try {
        var query = args.join(" ");
        if(!query) msg.channel.send(new Discord.MessageEmbed().setTitle("HATA").setDescription("Wkipedia da Aranacak şeyi yazman Gerekiyor!").setColor("RED"))
       ...
       var data = body.query.pages[0];
			if (data.missing) return msg.channel.send(new Discord.MessageEmbed().setTitle("HATA").setDescription("Sonuç bulunamadı."))
        ...
        msg.channel.send(new Discord.MessageEmbed().setTitle("HATA").setDescription(error).setColor("RED"))
    }
